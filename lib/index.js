...
class DeploymentBucketPlugin {
  constructor(serverless, options) {
    ...
    if (this.deploymentBucket.name) {
      ...
      this.config.acceleration = get(this.config, 'acceleration', undefined)
  ...
  async putBucketAccelerateConfiguration(name, acceleration) {
    const params = {
      Bucket: name,
      AccelerateConfiguration: {
        Status: acceleration === true ? 'Enabled' : 'Suspended',
      }
    };
  ...
  async applyDeploymentBucket() {
    try {
      ...
      if (this.config.acceleration) {
        await this.putBucketAccelerateConfiguration(this.deploymentBucket.name, this.config.acceleration)
        this.serverless.cli.log(`Applied deployment bucket acceleration configuration`)
      }
